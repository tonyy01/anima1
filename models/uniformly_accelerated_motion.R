...
for (i in 0:(total_t / state_t)) {
  ...
  if (i > min_ms) {
    for (j in 1:n_athelete) {
      ...
      # check which split the athlete is in
      split_current <- s %/% 10 
      
      # if the current split exceeds the former split, former split 
      # should be the current split, and the acceleration rate 
      # should be changed too
      if (split_current[j] != split[j]) { 
